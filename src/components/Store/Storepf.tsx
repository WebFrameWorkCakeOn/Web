    ...
    <div className="mt-10 p-10 border-t border-gray-200 flex flex-col pt-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {cakes.map((cake, i) => (
          <div
            key={i}
            className="flex flex-col items-center w-full aspect-4/3 border border-gray-100 rounded-lg shadow-sm bg-white pb-4"
          >
            <img
              src={cake.img}
              alt=""
              className="object-cover w-full h-70 rounded-t-lg justify-center items-center"
            />
            <div className="w-11/12 mt-3 flex flex-col items-center gap-1">
              <Link
                to="/order-detail"
                state={{ cakeImage: cake.img, cakeNotice: cake.notice }}
                className="w-full bg-black text-white text-sm rounded py-1.5 transition hover:bg-black/80 text-center block"
              >
                주문하기
              </Link>
              {cake.notice && (
                <div className="flex flex-row w-full text-xs mt-1 text-red-400 bg-red-50 border border-red-200/80 px-2 py-1 text-center rounded">
